...
describe('AuthService', () => {
  ...
  it('FE redirect success created', async () => {
    ...
    const result = authService.createFeUrlRedirect({
      accessToken: '123',
      refreshToken: '456',
    });
  ...
  it('FE redirect error created', async () => {
    ...
    const result = authService.createFeUrlErrorRedirect(
      EAuthError.InternalServer,
    );
    return expect(result).toBe(url);
  });
  ...
  it('Login with google', async () => {
    ...
    const decodeRefresh = jwt.verify(result.refreshToken, secretRefresh) as {
      id: string;
    };
    const decodeAccess = jwt.verify(result.accessToken, secretAccess) as {
      id: string;
    };
    ...
    return expect([accessId, refreshId]).toEqual([
      dataId._id.toString(),
      dataId._id.toString(),
    ]);
  });
